// app/api/admin/events/rejected/route.ts
...
export async function GET(request: Request) {
  try {
    ...
    // Build where clause for rejected events
    const where: any = { 
      status: "REJECTED"
    }
    ...
    // Get rejected events with pagination
    ...
    const transformedEvents = events.map(event => ({
      ...
      leadsCount: 0, // Rejected events won't have leads
